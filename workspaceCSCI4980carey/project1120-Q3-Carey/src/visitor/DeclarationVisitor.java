...
public class DeclarationVisitor extends ASTVisitor {
	...
	private GNode insertTypeNode(TypeDeclaration typeDecl) {
		...
		String type = rBinding.toString().contains("final ") ? "final" : rBinding.toString().contains("public ") ? "public" : rBinding.toString().contains("private ") ? "private" : "static";

		String typeName = typeDecl.getName().getFullyQualifiedName();
		String id = prjName + "." + pkgName + "." + typeName;
		GClassNode n = new GClassNode(id, typeName, prjName + "." + pkgName, type);
	...
	private GNode insertMethodNode(MethodDeclaration methodDecl) {
		...
		GMethodNode n = new GMethodNode(id, methodName, parent, type);
	...
	private void addConnection(GNode srcNode, GNode dstNode, int offset) {
		...
		String conLabel = dstNode.getDeclType(); // "offset: " + offset;
